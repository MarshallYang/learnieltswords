...
面向冲刺 IELTS Band 7-8 的中文学习者本地词汇训练器：约 7500 个核心/学术词，基于简化 SM-2 间隔重复，进度保存在浏览器，纯前端。
...
| 模块 | 说明 |
|------|------|
| 今日学习 | 先到期复习，再学新词；默认每天 20 新词（可调 10/15/20/30） |
| 单词本 | 搜索、按分层/学习状态筛选 |
| 统计 | 掌握度、今日评分分布、近两周归档 |
...
重新生成：bun run build:words（见 scripts/build-words.mjs）

数据来源：qwerty-learner 雅思相关词表、LynnShaw vocabulary_for_ielts、CET-6、TOEFL 公开列表（去重合并）。扩展时可增加源文件、补例句、校对高频释义。
